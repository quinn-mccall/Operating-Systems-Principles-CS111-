...
class TestLab1(unittest.TestCase):
    ...
    def test_8_commands(self):
        self.assertTrue(self.make, msg='make failed')
        cl_result = subprocess.run(('ls | cat | wc | sort | uniq | head | tail | rev'), capture_output=True, shell=True)
        pipe_result = subprocess.check_output(('./pipe', 'ls', 'cat', 'wc', 'sort', 'uniq', 'head','tail','rev'))
        self.assertEqual(cl_result.stdout, pipe_result, msg=f"The output from ./pipe should be {cl_result.stdout} but got {pipe_result} instead.")
        self.assertTrue(self._make_clean, msg='make clean failed')
    ...
    def test_no_command(self):
        self.assertTrue(self.make, msg='make failed')
        pipe_result = subprocess.run(('./pipe'), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.assertEqual(pipe_result.returncode, 22, msg="Expected EINVAL return code (22) when no arguments are passed.")
        self.assertNotEqual(pipe_result.stderr, '', msg="Error message should be written to stderr when no arguments are passed.")
        self.assertTrue(self._make_clean, msg='make clean failed')
